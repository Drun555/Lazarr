...
flowchart LR
    ...
    Probe --> Worker
    Worker --> DB
    Subtitles --> SP[SubtitleProvider]
    SP --> Assets[Внешние субтитры MediaAsset]
    Assets --> DB
```
...
- `Task` хранит требования, автора, сезон и признак отслеживания всего сезона.
...
- `MediaAsset` уникален по загрузке и индексу видео. `SubtaskAsset` хранит собственные preflight/verification и ручное исключение; разные задачи могут выбрать разные внешние дорожки одного видео.
- Флаги `pending` и `current` отделяют загрузку улучшения от последней проверенной версии.
...
Библиотеки и Jellyfin-совместимый API читают только проверенные текущие версии. `SubtitleService` дополняет `MediaAsset.tracks` внешними файлами и обновляет незакрытые языковые потребности подтасок; транскодинг не выполняется.
...
Удаление задач синхронизируется с поиском и монитором. Связи других задач защищают общие Downloads и MediaAssets. Сначала движок подтверждает удаление торрента, затем транзакция удаляет связи задачи и сохраняет журнал `cleanup.*`; очистка каталогов повторяется до успеха. Повторная загрузка того же infohash ждёт окончания очистки.
