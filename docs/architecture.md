...
Библиотеки и Jellyfin-совместимый API читают проверенные версии через `LibraryAsset`. `SubtitleService` дополняет `MediaAsset.tracks` внешними файлами и обновляет незакрытые языковые потребности подтасок; транскодинг не выполняется.
