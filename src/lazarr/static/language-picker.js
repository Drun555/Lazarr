...
function languagePicker(name, label, values, help) {
  ...
  return `<div class="language-field"><label id="${id}-label" for="${id}-input">${esc(label)}</label><div class="language-picker" data-language-picker="${name}"><div class="language-values">${selected.map(code=>languagePill(name,code)).join('')}<input id="${id}-input" class="language-query" type="text" placeholder="Добавить язык…" role="combobox" aria-autocomplete="list" aria-expanded="false" aria-controls="${id}-options" aria-labelledby="${id}-label" autocomplete="off" spellcheck="false"></div><div id="${id}-options" class="language-options" role="listbox" aria-labelledby="${id}-label" hidden></div></div>${help?`<small>${esc(help)}</small>`:''}</div>`;
}
